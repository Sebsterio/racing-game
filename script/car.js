...
function carClass() {
	...
	this.carY;
	this.carZ;
	this.accZ; // vertical acceleration
	this.carSpeed;
	this.carAng;
	...
	this.moveCar = function() {
		...
		/* jumping */
		if (currentTile == TRACK_RAMP && this.carZ == 0){
			this.accZ = this.carSpeed;
		}
		if (this.accZ != 0 || (this.accZ == 0 && this.carZ != 0)) {
			this.carZ = this.carZ + this.accZ
			this.accZ = this.accZ - GRAVITY;					
			if (this.carZ <= 0) {
				this.carZ = 0;
				this.accZ = 0;
			}
		...
		if (this.carZ > 0 || nextTile == TRACK_ROAD || nextTile == TRACK_OIL ||
			...
			this.carSpeed = 0;
			if (currentTile == TRACK_WALL || this.carX < 0 || this.carX > canvas.width ||
				  this.carY < 0 || this.carY > canvas.height) {
				setTimeout(function(){
					this.resetCar()
				}.bind(this), 1000)  // 
			}
